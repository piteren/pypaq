...
def test_point_normalized():
    ...
    assert paspa.is_from_space(pa)
    print(paspa.is_from_space(pb))
    assert paspa.is_from_space(pb)

    pan = paspa.point_normalized(pa)
    print(pan)
    assert pan == {'a': 0.0, 'b': 0.15000000000000002, 'c': 0.5, 'd': 0.625}
    pbn = paspa.point_normalized(pb)
    print(pbn)
    assert pbn == {'a': 0.55, 'b': 0.45, 'c': 0.8333333333333333, 'd': 0.125}
...
def test_loop():
    ...
    for _ in range(100):
        ...
        assert paspa_a.rdim <= paspa_a.dim
        paspa_b = PaSpa(get_subs(SAMPLE_PSDD))
        assert paspa_b.rdim <= paspa_b.dim
