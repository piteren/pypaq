...
class OMPRunner_BLOCKING:
    ...
    def _build_and_start_RW(self, id:int):
        ...
        self.rwwD[id]['rww'] = OMPRunner_BLOCKING.RWWrap(
            ique=                   Que(),
            oque=                   self.que_RW,
            id=                     id,
    ...
    def process(
            self,
            tasks: List[dict],
            exit=   True        # exits OMP when process() finished
    ) -> List[Any]:
        ...
        while len(resultsD) < num_tasks:
            ...
            for msg in rww_msgL:
                ...
                # RWWrap exception
                    ...
                    rww_id = msg.data
                    if self.verb>0 or self.print_exceptions: print(f' > {self.omp_name} received exception message from RW: {msg.type}, not finished task ix: {rww_tasks[rww_id][0]}; .. recreating RWWrap and putting task again')
                    ...
                    self._build_and_start_RW(rww_id)
                    msg = QMessage(type='task', data=rww_tasks[rww_id][1])
                    self.rwwD[rww_id]['rww'].ique.put(msg)
